...
<details id="menu_details">
    ...
    <?php foreach($pages as $page): ?><a id="nav-<?= $page->Hash ?>" href="<?= $scriptURL.'/pg?'.$page->Hash ?>"><?= $page->Name ?></a><?php endforeach;?>
</details>
<div id="menu_top" class="topnav"><nav id="nav-page-list"> 
<?php foreach($pages as $page): ?><a id="menu-<?= $page->Hash ?>" href="<?= $scriptURL.'/pg?'.$page->Hash ?>"><?= $page->Name ?></a><?php endforeach;?>
</nav></div>
